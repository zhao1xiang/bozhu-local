...
const EmbedAppointmentPlain: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const outpatient_number = searchParams.get('outpatient_number') || '';
    const phone = searchParams.get('phone') || '';
    const diagnosis = searchParams.get('diagnosis') || '';
    const drug_name = searchParams.get('drug_name') || '';
    const eye = searchParams.get('eye') || '';
    const injection_count = parseInt(searchParams.get('injection_count') || '0');
    ...
    if (!phone) missing.push('phone（联系方式）');
    if (!diagnosis) missing.push('diagnosis（诊断）');
    if (!drug_name) missing.push('drug_name（用药）');
    if (!eye) missing.push('eye（治疗眼）');
    if (!doctor) missing.push('doctor（注药医生）');
