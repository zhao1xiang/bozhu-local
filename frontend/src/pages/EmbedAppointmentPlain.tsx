...
interface AppItem {
  id?: string;
  appointment_date?: Dayjs | string;
  follow_up_date?: Dayjs | string;
...
// 从给定日期向后找最近的玻注日
const getNearestDate = (base: Dayjs, weekdays: string[]): Dayjs => {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (allowed.includes(weekday)) return candidate;
  }
  return base;
};

// 根据针次计算下一次预约基准日期
const getNextDate = (base: Dayjs, injectionCount: number, intervalDays: number): Dayjs => {
...
const EmbedAppointmentPlain: React.FC = () => {
  ...
  const [bozhuAppts, setBozhuAppts] = useState<AppItem[]>([
    ...
    { injection_count: 2, is_new: false },
    { injection_count: 3, is_new: false },
    { injection_count: 4, is_new: false },
  ]);

  const [periAppt, setPeriAppt] = useState<AppItem>({ injection_count: 1, is_new: false });
  ...
  useEffect(() => {
    ...
    const outpatient_number = clean(searchParams.get('outpatient_number'));
    const phone = clean(searchParams.get('phone'));
    const diagnosis = clean(searchParams.get('diagnosis'));
    const drug_name = clean(searchParams.get('drug_name'));
    const eyeRaw = clean(searchParams.get('eye'));
    const eye = eyeRaw === '' ? '双眼' : eyeRaw;  // 眼别无值默认双眼
    const injection_count_raw = clean(searchParams.get('injection_count'));
    const injection_count = injection_count_raw ? (parseInt(injection_count_raw) || 0) : 0;
    const doctor = clean(searchParams.get('doctor'));

    // 验证必填（只有姓名和住院号是必须的）
    const missing: string[] = [];
    if (!name) missing.push('name（姓名）');
    if (!outpatient_number) missing.push('outpatient_number（住院号）');

    if (missing.length > 0) {
      setError(`缺少必填参数：${missing.join('、')}`);
      setLoading(false);
      return;
    }
    ...
    const pd: PatientData = {
      ...
      injection_count,
      doctor,
      patient_type: injection_count >= 1 ? '经治' : '初治',
    };
    setPatientData(pd);
    ...
    Promise.all([
      ...
      outpatient_number
        ? apiClient.get('/embed/verify-plain', { params: { name, outpatient_number, phone, diagnosis, drug_name, eye, injection_count, doctor } }).catch(() => null)
        : Promise.resolve(null),
    ]).then(([wdRes, intRes, verifyRes]) => {
      const wd = wdRes.data?.value || '1';
      setInjectionWeekdays(wd.split(',').filter(Boolean));
      setInjectionIntervalFirst4(parseInt(intRes.data?.value || '30'));
      ...
      if (verifyRes?.data) {
        const { patient, appointments } = verifyRes.data;
        if (patient?.id) {
          setPatientData(prev => ({ ...prev, id: patient.id }));
        }
        if (Array.isArray(appointments) && appointments.length > 0) {
          const bozhuList = appointments
            ...
            .map((a: any) => ({
              ...a,
              appointment_date: a.appointment_date ? dayjs(a.appointment_date) : undefined,
          ...
          if (periFound) {
            setPeriAppt({
              ...periFound,
              appointment_date: periFound.appointment_date ? dayjs(periFound.appointment_date) : undefined,
              follow_up_date: periFound.follow_up_date ? dayjs(periFound.follow_up_date) : undefined,
              is_new: false,
            });
          } else {
            // 没有围手术期记录，按系统逻辑生成默认值
  ...
  const generateDates = (weekdays: string[], intervalDays: number): AppItem[] => {
    ...
    for (let i = 0; i < 4; i++) {
      const cnt = i + 1;
      const date = getNearestDate(currentBase, weekdays);
      result.push({ injection_count: cnt, appointment_date: date, follow_up_date: date, treatment_phase: '强化期', time_slot: '上午', condition_status: '稳定' });
      currentBase = getNextDate(date, cnt + 1, intervalDays);
    }
    return result;
  };
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      const saveRes = await apiClient.post('/embed/save', buildPayload('embed_plain_direct'));
      if (saveRes.data?.patient?.id) {
        setPatientData(prev => ({ ...prev, id: saveRes.data.patient.id }));
      }
  ...
  const handleSaveAndPrint = async () => {
    setPrintSaving(true);
    try {
      const saveRes = await apiClient.post('/embed/save', buildPayload('embed_plain_print'));
      if (saveRes.data?.patient?.id) {
        setPatientData(prev => ({ ...prev, id: saveRes.data.patient.id }));
      }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', background: colors.blueBg }}>
        <Spin size="large" />
        <div style={{ marginTop: 12, color: colors.blue }}>正在加载...</div>
      </div>
    );
  }
  ...
  return (
    <div style={{ background: colors.blueBg, minHeight: '100vh', padding: '16px' }}>
      <div style={{ maxWidth: 860, margin: '0 auto', background: '#fff', borderRadius: 12, padding: '16px 24px', boxShadow: '0 1px 6px rgba(0,0,0,0.12)', border: `1px solid ${colors.blueBorder}` }}>
        ...
        {savedOk && (
          <Alert type="success" message="预约保存成功" showIcon style={{ marginBottom: 12, borderRadius: 8 }} />
        )}
            ...
            {bozhuAppts.map((appt, idx) => (
              <div key={idx} style={{ background: '#fafafa', borderRadius: 8, padding: '10px 16px', marginBottom: 8, border: '1px solid #e8e8e8', position: 'relative' }}>
                ...
                <Row gutter={12} align="middle">
                  ...
                  <Col span={5}>
                    <span style={{ fontSize: 12, color: '#888' }}>玻注日期</span>
                    <DatePicker
                      size="small"
                      style={{ width: '100%' }}
                  ...
                  <Col span={4}>
                    <span style={{ fontSize: 12, color: '#888' }}>时间段</span>
                    <Select
                      size="small"
                      style={{ width: '100%' }}
                  ...
                  <Col span={4}>
                    <span style={{ fontSize: 12, color: '#888' }}>针次</span>
                    <InputNumber
                      size="small"
                      min={1}
                      style={{ width: '100%' }}
                  ...
                  <Col span={4}>
                    <span style={{ fontSize: 12, color: '#888' }}>状况</span>
                    <Select
                      size="small"
                      style={{ width: '100%' }}
                  ...
                  <Col span={5}>
                    <span style={{ fontSize: 12, color: '#888' }}>阶段</span>
                    <Select
                      size="small"
                      style={{ width: '100%' }}
...
export default EmbedAppointmentPlain;
